...
Esta aplicación automatiza el análisis estadístico de archivos Excel para 
evaluar la linealidad de un método analítico. Se realizan dos análisis:
  - App 1: Análisis de Linealidad mediante OLS, evaluación de residuales y 
    generación de gráficos (OLS, Q-Q y gráfico de escala).
  - App 2: Test de Significancia y evaluación de falta de ajuste del modelo lineal,
    con opción de reanálisis mediante transformación (Box-Cox o Yeo-Johnson).

Instrucciones de uso:
  1. Se puede seleccionar un archivo de prueba para listar los datos.
  2. Seleccione la carpeta que contiene los archivos Excel a procesar.
  3. Haga clic en "Procesar" para ejecutar ambos análisis; los resultados se 
     guardarán en cada archivo Excel.
"""
...
def escribir_encabezado_combinado(ws, row, col_start, col_end, texto, fill_color="FFD966"):
    """Combina celdas de encabezado y asigna formato."""
...
def escribir_tabla_dict(ws, start_row, start_col, dict_data, titulo=None):
    """
    Escribe un diccionario en formato tabla en la hoja de Excel.
    Retorna la siguiente fila después de la tabla.
    """
    thin_border = Border(
...
def escribir_texto_multilinea(ws, start_row, col, texto, font_size=10):
    """
    Escribe un texto multilínea en la hoja de Excel, una línea por celda.
    Retorna la siguiente fila luego del bloque.
    """
...
def insertar_imagen_excel(ws, image_path, cell_address):
    """
    Inserta una imagen en la hoja de Excel, si existe.
    """
...
def limpiar_hoja(ws):
    """
    Limpia la hoja de Excel (desde la fila 40 en adelante) removiendo celdas fusionadas y
...
def realizar_regresion_OLS(df, usar_robusto=False, transformar=False):
    """
    Realiza una regresión lineal OLS (opcionalmente robusta o con transformación logarítmica).
    """
...
def analizar_residuales(modelo):
    """
    Analiza los residuales del modelo utilizando Shapiro-Wilk, Durbin-Watson,
    Breusch-Pagan y White. Retorna un diccionario con los resultados.
    """
    resid = modelo.resid
    sw = shapiro(resid)
...
def extraer_stats_ols(modelo):
    """
    Extrae los coeficientes y medidas de ajuste del modelo OLS.
    Retorna dos diccionarios: uno con coeficientes y otro con estadísticas de ajuste.
    """
    p = modelo.params
    pv = modelo.pvalues
...
def generar_grafico_y_guardar(df, modelo, titulo, filename):
    """
    Genera un gráfico con tres subgráficos:
      1. Scatterplot con la línea OLS.
      2. Q-Q plot de los residuales.
      3. Gráfico de escala (√|Residuales estandarizados| vs. valores ajustados).
    Guarda la figura en el archivo 'filename'.
    """
...
def calcular_correlacion_ttest(df):
    """
    Calcula el coeficiente de correlación, R², el estadístico t y su comparación
    con el valor crítico para determinar la significancia de la correlación.
...
def analizar_segmento(df, etiqueta_segmento, ws, row_excel):
    """
    Realiza el análisis OLS para un segmento (por ejemplo, datos negativos o positivos),
    escribe los resultados y gráficos en la hoja de Excel, y retorna la fila siguiente.
    """
    if df.shape[0] < 2:
...
def analizar_ols_por_segmentos(ruta_excel):
    """
    Ejecuta el análisis OLS por segmentos (datos negativos y positivos) y escribe los
    resultados en la hoja de Excel.
    """
    try:
...
def format_p_value(p):
    """
    Formatea el p-value para mostrarlo en notación científica si es necesario.
    """
...
def lack_of_fit_test(df, xcol, ycol):
    """
    Realiza el test de falta de ajuste (lack-of-fit) para el modelo lineal.
    """
    df_ = df.copy()
    formula = f"{ycol} ~ {xcol}"
...
def process_file(file_path):
    """
    Procesa un archivo Excel para el test de significancia de linealidad,
    eliminando outliers, realizando los cálculos y guardando los resultados
    en una hoja nueva del archivo.
    """
    try:
...
def listar_datos_prueba(ruta_archivo):
    """
    Carga y muestra por consola los datos de la hoja OLS para verificación.
    """
    try:
...
def prueba_listado_datos():
    """
    Ejecuta la prueba de listado de datos solicitando un archivo Excel.
    """
    root = Tk()
    root.withdraw()
...
def main():
    """
    Función principal que ejecuta la prueba de listado de datos, el análisis de linealidad (App 1)
    y el test de significancia (App 2) en todos los archivos Excel de la carpeta seleccionada.
    """
...
def iniciar_interfaz():
    """
    Crea y configura la interfaz gráfica principal para la aplicación,
    integrando mejoras en el tamaño de ventana, instrucciones, selección de archivo
    y carpeta, y mensajes de estado.
    """
    root = tk.Tk()

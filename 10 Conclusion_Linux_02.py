...
def process_file(file_tuple):
    ...
    try:
        ...
        # Se extrae la información desde la celda E7 (rango E7:M94)
        ...
        data_range_significante= df_linealidad.iloc[0:205, 0:3]  # filas 0 a 205 columnas A a D
        #data_range_significante = df_significante  # Se extrae toda la información empezando en A1
        ...
        ws["B60"] = mensaje_linealidad
        ws["B60"].alignment = Alignment(horizontal="left", vertical="justify", wrap_text=True)
        ws["B60"].font = Font(size=9)

        ws["B72"] = mensaje_significante
        ws["B72"].alignment = Alignment(horizontal="left", vertical="justify", wrap_text=True)
        ws["B72"].font = Font(size=9)

        ws["B86"] = mensaje_repetibilidad
        ws["B86"].alignment = Alignment(horizontal="left", vertical="justify", wrap_text=True)
        ws["B86"].font = Font(size=9)

        ws["B98"] = mensaje_precision
        ws["B98"].alignment = Alignment(horizontal="left", vertical="justify", wrap_text=True)
        ws["B98"].font = Font(size=9)

        ws["B109"] = mensaje_veracidad
        ws["B109"].alignment = Alignment(horizontal="left", vertical="justify", wrap_text=True)
        ws["B109"].font = Font(size=9)

        ws["B120"] = mensaje_robustez
        ws["B120"].alignment = Alignment(horizontal="left", vertical="justify", wrap_text=True)
        ws["B120"].font = Font(size=9)

        ws["B131"] = mensaje_incertidumbre
        ws["B131"].alignment = Alignment(horizontal="left", vertical="justify", wrap_text=True)
        ws["B131"].font = Font(size=9)
...
for result in results:
    if len(result) >= 9:
        ...
        contenido = (
            f"[bold]Linealidad:[/bold]\n{mensaje_linealidad}\n\n"
            f"[bold]Significante:[/bold]\n{mensaje_significante}\n\n"
            f"[bold]Repetibilidad:[/bold]\n{mensaje_repetibilidad}\n\n"
            f"[bold]Precisión:[/bold]\n{mensaje_precision}\n\n"
            f"[bold]Veracidad:[/bold]\n{mensaje_veracidad}\n\n"
            f"[bold]Robustez:[/bold]\n{mensaje_robustez}\n\n"
            f"[bold]Incertidumbre:[/bold]\n{mensaje_incertidumbre}"
        )

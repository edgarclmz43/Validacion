...
def copiar_hoja(hoja_origen, wb_destino, nuevo_nombre):
    """
    Copia el contenido y el formato completo de hoja_origen a una nueva hoja en wb_destino
    con el nombre nuevo_nombre. Se copia de forma general toda la estructura interna de celdas,
    lo que incluye valores, fórmulas y estilos. De esta forma, si una celda contiene una fórmula,
    esta se mantendrá intacta.
    
    También se copian dimensiones de filas y columnas, celdas fusionadas, congelación de paneles
    y área de impresión.
    """
    hoja_dest = wb_destino.create_sheet(title=nuevo_nombre)
    
    # Copiar todas las celdas de la hoja de forma general (incluyendo fórmulas y estilos)
    hoja_dest._cells = {}
    for coord, celda in hoja_origen._cells.items():
        nueva_celda = copy(celda)
        nueva_celda.parent = hoja_dest  # Actualizar el padre a la nueva hoja
        hoja_dest._cells[coord] = nueva_celda
...
def procesar_archivo(ruta_archivo, wb_plantilla):
    ...
    if hoja_plan_origen and hoja_informe_origen:
        ...
        # Copiar las hojas de la plantilla al libro actual utilizando la copia general
        ...
        for (fila, col), valor in datos_extraidos.items():
            # Se sobreescribe el valor manteniendo el formato y fórmulas (si las hubiera) copiadas de la plantilla

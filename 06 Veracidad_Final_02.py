...
def procesar_archivos(skiprows_values, output_directory):
    ...
    # Definición de las columnas a leer basándose en las etiquetas
    columnas_etiq = ["Lubp1", "Lubp2", "Lubp3", "Lubp4", "Lubp5", "Lubp6", "Lubp7", "Lubp8", "Lubp9", "Lubp10", "LP", "u"]
    ...
    for idx, skiprows in enumerate(skiprows_list, start=1):
        ...
        for i, archivo in enumerate(archivos):
            try:
                # Abrir el libro de forma que se pueda determinar la fila de cabecera
                wb_temp = load_workbook(archivo, read_only=True)
                ws_temp = wb_temp["Registro"]
                header_row = skiprows + 1  # la fila de cabecera según el skiprows
                # Buscar en la fila de cabecera la columna donde se encuentre "Lubp1"
                col_idx = None
                for cell in ws_temp[header_row]:
                    if cell.value == "Lubp1":
                        col_idx = cell.column
                        break
                if col_idx is None:
                    raise ValueError("No se encontró la etiqueta 'Lubp1' en la fila de cabecera.")
                
                nrows = 0
                # Iterar desde la fila siguiente a la cabecera para contar hasta encontrar una celda vacía
                for cell_tuple in ws_temp.iter_rows(min_row=header_row+1, min_col=col_idx, max_col=col_idx, values_only=True):
                ...
                datos = pd.read_excel(
                    ...
                    skiprows=skiprows,
                    nrows=nrows,
                    usecols=columnas_etiq,
                    engine="openpyxl"
                )
                # Ya no es necesario renombrar columnas puesto que se leen por etiqueta.

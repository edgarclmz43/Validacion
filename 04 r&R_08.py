...
def formato_valor(valor):
    """
    Formatea el valor para presentación, mostrando dos decimales.
    """
...
def generar_informe(file_path):
    ...
    # Desfusionar el rango B115:L150 (si está fusionado) para evitar errores de escritura
    try:
        ws.unmerge_cells("B115:L150")
    except Exception as e:
        print(f"No se pudo desfusionar el rango B115:L150: {e}")

    # Dividir el informe en líneas y escribirlas desde la celda B115
    lines = informe.split('\n')
    start_row = 115  # Comenzar en la fila 115, columna B (columna 2)

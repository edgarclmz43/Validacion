...
# Se requiere xlwings para actualizar solo la hoja "Robustez"
...
###############################################################################
#   SECCIÓN 2: ACTUALIZACIÓN DE HOJA "ROBUSTEZ" SIN AFECTAR LAS DEMÁS HOJAS    #
#                           (usando xlwings)                                  #
###############################################################################
...
def pegar_datos_destino(carpeta_destino, combinados):
    ...
    comience con el prefijo indicado. Con xlwings se abre el archivo y se actualiza
    ...
    for reg in combinados:
        ...
        for root, _, files in os.walk(carpeta_destino):
            for file in files:
                if file.lower().endswith(('.xlsm', '.xlsx', '.xls')) and file.startswith(prefijo):
        ...
        try:
            ...
            wb = xw.Book(destino_encontrado)
        except Exception as e:
            print(f"Error al abrir archivo destino {destino_encontrado}: {e}")
            continue

        try:
            # Si existe la hoja "Robustez", se usa; de lo contrario, se agrega
            if "Robustez" in [s.name for s in wb.sheets]:
                ws_dest = wb.sheets["Robustez"]
            else:
                ws_dest = wb.sheets.add("Robustez")

            # Datos comunes
            ws_dest.range("H9").value = reg["Temperatura_max"]
            ws_dest.range("H10").value = reg["Temperatura_min"]
            ws_dest.range("D13").value = reg["Certificado_max"]
            ws_dest.range("F13").value = reg["Certificado_min"]
            ws_dest.range("D12").value = "Temperatura Alta"
            ws_dest.range("F12").value = "Temperatura Baja"
            ws_dest.range("D14").value = ""
            ws_dest.range("F14").value = ""

            # Pegar block_max en columnas C, D y E (desde fila 17)
            fila = 17
            for r in reg["block_max"]:
                ws_dest.range((fila, 3)).value = r["LPatron"]
                ws_dest.range((fila, 4)).value = r["EA"]
                ws_dest.range((fila, 5)).value = r["Uexp"]
                fila += 1

            # Pegar block_min en columnas F, G y H (desde fila 17)
            fila = 17
            for r in reg["block_min"]:
                ws_dest.range((fila, 6)).value = r["EA"]
                ws_dest.range((fila, 7)).value = r["Uexp"]
                fila += 1
...
def procesar_archivo_evaluacion(ruta_archivo):
    """
    Utiliza xlwings para abrir el archivo Excel y actualizar únicamente la hoja "Robustez"
    ...
    try:
        if "Robustez" in [s.name for s in wb.sheets]:
            ws = wb.sheets["Robustez"]
        else:
            ws = wb.sheets.add("Robustez")

        # Escribir resultados en celdas predefinidas
        ws.range("B31").value = "-------------------------------------------------------------------------------"
        ws.range("B32").value = "Prueba ANOVA de una vía"
        ws.range("B33").value = "-------------------------------------------------------------------------------"
        ws.range("B34").value = "Estadístico F:"
        ws.range("C34").value = f_stat
        ws.range("B35").value = "Valor p:"
        ws.range("C35").value = p_value
        ws.range("B36").value = "Nivel de significancia:"
        ws.range("C36").value = nivel_significancia
        ws.range("B37").value = "Conclusión de la prueba:"
        ws.range("B37:K37").api.Merge()
        ws.range("B37").value = resultado

        temp_min = ws.range("H10").value
        temp_max = ws.range("H9").value
        ...
        ws.range("B43").value = conclusion_text
